...
from agents import run_context_agent, run_ops_agent, run_planner_agent, run_report_agent, run_audit_agent
...
def node_audit(state: AppState) -> AppState:
    feedback = run_audit_agent(state.get("business_context", ""), state.get("dispatch_plan", ""))
    print("\n===== AUDIT FEEDBACK =====")
    print(feedback)
    print("==========================\n")
    passed = feedback.strip() == "PASS"
...
def build_graph():
    ...
    def route_audit(state: AppState) -> str:
        if not state.get("audit_passed", False):
            return "planner"
        risk_score = state.get("weather_risk", {}).get("route_risk_score_0_3", 0)
        # For single location fallback
        if risk_score == 0 and "risk_score_0_3" in state.get("weather_risk", {}):
    ...
    g.add_conditional_edges("audit", route_audit)
