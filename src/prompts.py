...
PLANNER_PROMPT = ChatPromptTemplate.from_messages([
    ...
    ("user",
     ...
     "If you previously failed the audit, fix the plan based on this feedback:\n"
     "Audit Feedback: {audit_feedback}\n")
])

AUDIT_PROMPT = ChatPromptTemplate.from_messages([
    ("system",
     "You are AuditAgent, a strict quality assurance checker. "
     "Your job is to review the proposed dispatch plan against the business context (safety rules, SLA constraints, etc.).\n"
     "If the dispatch plan violates ANY rules in the business context, you must return 'FAIL: [Explanation of what is wrong and what to fix]'.\n"
     "If it perfectly aligns with all rules, return exactly 'PASS'."),
    ("user",
